...
/**
 * ¿La conexión soporta prepared statements?
 *
 * El pooler de Supabase en modo TRANSACCIÓN (puerto 6543) no los soporta:
 * multiplexa varias sesiones sobre la misma conexión, así que un statement
 * preparado en una consulta puede no existir en la siguiente. postgres.js los
 * usa por default, y el síntoma es un `prepared statement "s1" does not exist`
 * intermitente — aparece bajo carga, no en la primera prueba.
 *
 * El modo SESIÓN (puerto 5432) y la conexión directa sí los soportan.
 *
 * @param {string} url
 * @returns {boolean}
 */
export function preparedStatements(url) {
  try {
    const parsed = new URL(url);
    // Convención heredada de PgBouncer, la respetan varios proveedores.
...
export function pgOptions(url, extra = {}) {
  return {
    ssl: sslFor(url),
    prepare: preparedStatements(url),
